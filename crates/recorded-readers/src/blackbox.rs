//! Offline blackbox decoding and record preservation. See the recorded-input guide.
...
mod tests {
    ...
    fn profile_names_do_not_change_semantics_and_boot_time_is_not_wall_time() {
        let base = r#"{"profile_id": "betaflight_blackbox_csv", "version": "1", "family": "fpv", "source_role": "fc", "format": "blackbox_decoded_csv", "extensions": ["csv"], "default_clock_basis": "host_received", "channels": ["onboard", "link_stats"], "fields": {"time": "time", "link": ["customLink"]}}"#;
        let a = crate::profile::parse_profile(base).expect("profile");
    ...
    fn unit_suffix_is_stripped_and_rx_loss_is_stale() {
        let p = crate::profile::parse_profile(
            r#"{"profile_id": "bb", "version": "1", "family": "fpv", "source_role": "fc", "format": "blackbox_decoded_csv", "extensions": ["csv"], "default_clock_basis": "boot_relative", "fields": {"time": "time", "status": ["rxSignalReceived", "rxFlightChannelsValid", "failsafePhase"], "link": ["rssi"], "gps": ["GPS_numSat", "GPS_coord[0]", "GPS_coord[1]"]}}"#)
        .expect("profile");
