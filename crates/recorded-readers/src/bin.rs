//! Offline bin decoding and record preservation. See the recorded-input guide.
...
mod tests {
    ...
    fn nonfinite_records_keep_neighbors_bits_and_untimed_accounting() {
        let profile = crate::profile::parse_profile(include_str!(
            "../../../profiles/recorded/public/ardupilot_plane_bin.json"
        ))
        .unwrap();
    ...
    fn fmt_length_mismatch_and_truncated_record_are_malformed() {
        let p = crate::profile::parse_profile(
            r#"{"profile_id": "b", "version": "1", "family": "ugv", "source_role": "fc", "format": "ardupilot_dataflash_bin", "extensions": ["bin"], "default_clock_basis": "boot_relative", "fields": {"time": "TimeUS"}}"#)
        .expect("profile");
