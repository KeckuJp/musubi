//! Offline edgetx decoding and record preservation. See the recorded-input guide.
...
mod tests {
    ...
    fn recorded_text_sensor_preserves_commas_and_text_type_through_existing_reader() {
        let p = crate::profile::parse_profile(TMR_PROFILE).unwrap();
    ...
    const TMR_PROFILE: &str = r#"{"profile_id": "h", "version": "1", "family": "fpv", "source_role": "handset", "format": "edgetx_csv", "extensions": ["csv"], "default_clock_basis": "host_received", "fields": {"time": "tmr10ms", "link": ["RQly(%)"]}}"#;

    #[test]
    fn tmr10ms_variant_reads_as_boot_relative_and_missing_time_is_malformed() {
        let p = crate::profile::parse_profile(TMR_PROFILE).expect("profile");
    ...
    fn profile_time_mapping_is_not_inert() {
        ...
        let profile = crate::profile::parse_profile(&incompatible).expect("profile");
    ...
    fn explicit_time_alternative_rejects_ambiguous_partial_and_missing_headers() {
        let composite = TMR_PROFILE.replace("tmr10ms", "Date,Time|tmr10ms");
        let profile = crate::profile::parse_profile(&composite).expect("profile");
        for input in [
