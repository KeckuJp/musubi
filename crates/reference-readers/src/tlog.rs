...
"#;
    ...
    #[test]
    fn gps_reported_fix_and_missing_quantities_preserve_raw_values() {
        let profile = parse_profile(PROFILE, "test").unwrap();
        for (fix, satellites, eph, label) in [
            (6u8, 12u8, 150u16, "RTK_FIXED"),
            (8, 0, 0, "PPP"),
            (255, 255, 65535, "UNKNOWN_RETAINED"),
        ] {
            let mut payload = [0u8; 30];
            payload[20..22].copy_from_slice(&eph.to_le_bytes());
            payload[28] = fix;
            payload[29] = satellites;
            let rows = TlogReader
                .read(&profile, &framed(24, &payload, false))
                .unwrap();
            let row = &rows[0];
            assert_eq!(
                crate::field(row, "GPS_RAW_INT.fix_type"),
                Some(&FieldValue::I64(i64::from(fix)))
            );
            assert_eq!(
                crate::field(row, "gps_fix_type_reported"),
                Some(&FieldValue::Text(label.into()))
            );
            assert_eq!(
                crate::field(row, "gps_satellites_visible_reported"),
                Some(&if satellites == 255 {
                    FieldValue::Blank
                } else {
                    FieldValue::I64(i64::from(satellites))
                })
            );
            assert_eq!(
                crate::field(row, "gps_hdop_reported"),
                Some(&if eph == 65535 {
                    FieldValue::Blank
                } else {
                    FieldValue::F64(f64::from(eph) / 100.0)
                })
            );
        }
    }
    ...
    fn explicit_gcs_recorded_clock_preserves_units_without_receipt_or_anchor_claim() {
        let mut p = parse_profile(PROFILE, "test").unwrap();
    ...
    fn unavailable_v2_extension_and_wrong_profile_do_not_become_claims() {
        ...
        let mut values = vec![
            ("MAVLINK.packet_sequence".into(), FieldValue::I64(1)),
            ("x".into(), FieldValue::I64(3)),
        ];
